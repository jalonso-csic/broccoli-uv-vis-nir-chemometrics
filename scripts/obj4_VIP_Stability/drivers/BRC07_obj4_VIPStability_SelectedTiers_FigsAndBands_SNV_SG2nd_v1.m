...
% ------------------------------------------------------------
% Objective 4 (Q1-ready): VIP stability figures + operational band export
%
...
%             (Tier 1 shown at the top, then Tier 2, then Tier 3; within-tier sorted
%              by pooled performance metric from Obj3 summaries).
...
% USER SETTINGS
% =========================
% If you know the Obj3 output folder, set it here (ASCII path preferred).
% Leave empty "" to auto-detect via common relative paths or via GUI.
OBJ3DIR_MANUAL = "";

% Output folder (required by manuscript workflow)
OUTDIR = fullfile(pwd, 'Objetivo_4');
if ~exist(OUTDIR,'dir'); mkdir(OUTDIR); end

% Selected endpoints (edit if desired)
% NOTE: Matching is robust to '.' vs '_' via makeValidName() logic.
Tier1_All = ["Antihypertensive_act.", "ABTS", "DPPH", "Total_phenolics", "Extraction_yield"];

% Selected Tier2 (chosen subset)
Tier2_Sel = ["SUM_GSL_indolic", "SUM_Amino_acids", "SUM_GSL_total"];

% Selected Tier3S (top-3 example)
Tier3_Sel = ["Methoxyglucobrassicin_2", "Glucobrassicin", "Methoxyglucobrassicin_1"];

% Sorting metric for within-tier ordering (descending)
SORT_METRIC_COL = "R2_pooled";   % change to "RPD_pooled" if needed

% Stability parameters
STAB_THR  = 0.70;    % stable if >= STAB_THR
...
if strlength(OBJ3DIR)==0 || ~(exist(OBJ3DIR,'dir')==7)
    ...
    % Try common relative locations from current folder (CELL array!)
    candDirs = { ...
    ...
    if ~found
        fprintf('\nOBJ3_OUT folder not found automatically.\n');
        tmp = uigetdir(pwd, 'Select your OBJ3_OUT folder');
        assert(~isequal(tmp,0), 'No OBJ3_OUT folder selected.');
        OBJ3DIR = string(tmp);
    end
end
...
%% =========================
% ORDER ENDPOINTS WITHIN EACH TIER (DESC by SORT_METRIC_COL)
% =========================
% CHANGED (minimal): look for SG2nd summaries
SUM_T1 = pickSummary(char(OBJ3DIR), '*OBJ3_Summary_Tier1*SG2nd*.xlsx',  'Tier1 Summary (SNV+SG2nd)');
SUM_T2 = pickSummary(char(OBJ3DIR), '*OBJ3_Summary_Tier2*SG2nd*.xlsx',  'Tier2 Summary (SNV+SG2nd)');
SUM_T3 = pickSummary(char(OBJ3DIR), '*OBJ3S_Summary_Tier3S*SG2nd*.xlsx', 'Tier3S Summary (SNV+SG2nd)');
...
% Final order for plotting: Tier1 (top), then Tier2, then Tier3 (bottom)
...
% =========================
% CHANGED (minimal): pick SG2nd VIPraw mats
...
%% =========================
% BUILD STABILITY MATRIX (Tier1 + Tier2 selected + Tier3 selected)
% =========================
...
%% =========================
% FIGURE 1 — HEATMAP (Tier1 top → Tier3 bottom)
% =========================
...
%% =========================
% FIGURE 2 — LINES (Tier3 selected indoles; already sorted by R2 desc)
% =========================
...
function xlsxPath = pickSummary(folder, pattern, label)
    cand = dir(fullfile(folder, pattern));
    if isempty(cand)
        warning('%s not found via pattern "%s" in %s. Keeping original order for that tier.', ...
...
function yOut = orderByMetric(yIn, summaryPath, metricCol)
    ...
    for i=1:numel(yIn)
        ...
        idx = strcmp(SY, q);
        if any(idx)
            r2(i) = S.(metricCol)(find(idx,1,'first'));
            continue;
        end
        ...
        idx2 = strcmp(SYv, qv);
        if any(idx2)
            r2(i) = S.(metricCol)(find(idx2,1,'first'));
            continue;
        end
    end

    r2tmp = r2;
    r2tmp(isnan(r2tmp)) = -inf;      % not found go last
    [~, ord] = sort(r2tmp, 'descend');
    yOut = yIn(ord);
end
