...
% ------------------------------------------------------------
% Objective 3 (audit-ready) - TWO FACTORS (NO N):
...
%   - Stratification: Maturity (outer folds)
...
% Standardise X after preprocessing
SCALE_X = true;
...
function runTier(tierName, yList)
    % Capture outer workspace variables
    ...
    'VariableNames', {'Tier','Config','Y','n', ...
        ...
        for iY = 1:numel(yList)
            ...
            VIP_STORE.(yn).foldPlan = foldPlan; %#ok<STRNU>
        end

        if varsProcessed == 0
            fprintf('  -> No variables were processed for %s under config %s. Check list names!\n', tierName, cfgName);
        else
            % Export per config only if variables were processed
    ...
    if isempty(allSummary)
        fprintf('\nSkipping compare workbook for %s because no variables were processed.\n', tierName);
        return;
    end
    ...
    % Optional wide-format quick view
    try
...
function [predLong, repMetrics, pooledMetrics, vipAll, lvAll] = ...
    ...
    for r = 1:R
        ...
        for k = 1:K
            ...
            % Preprocess (row-wise, no train statistics)
            Xtr_p = preprocessFcn(Xtr);
            Xte_p = preprocessFcn(Xte);
            
            % Optional column scaling using TRAIN statistics
            if params.scaleX
            ...
            % Inner-CV LV selection (minimum RMSE)
